...
print(' (see example 15.3.2)')

O = np.matrix(Ogrouped) ; n    = np.sum(O)
x = np.matrix([])

# construct x,y row matrices.
# yi is inverse error function: 
    # Φ(zi) =  Fi --> yi = Φ^[-1](Fi) = zi
# where zi  = -μ/σ + ai/σ  = B0 + B1*xi  ; xi = ai
# x is the upper interval ai
x = np.matrix([]) ; y = F = np.matrix([])
i = 0
while i < O.shape[1] :
    Fi = np.sum(O[0,:i])/n
    yi = Percentile(-5,Fi,Standard_Normal,1)
    y  = np.append(y,[[yi]], axis = 1)
    x  = np.append(x,[[Grouped_intvls[i-1][1]]], axis = 1)
    i += 1
